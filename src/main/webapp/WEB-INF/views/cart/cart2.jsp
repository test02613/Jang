              ...
              <tbody>
				...
				<c:when test="${fn:length(item) > 0}">
					<c:forEach items="${item }" var="item" varStatus="status">
						<input type="hidden" name="goods_att_amount" value="${item.itemname}">
						<input type="hidden" name="goods_att_amount" value="${item.itemcode}">
						<input type="hidden" name="goods_att_amount" value="${item.itemcost}">
						<tr>
							<td style="text-align:center">
                  				<input type="checkbox" name="chk" id="chk" value="${item.itemcode}">
                  			</td>
                  			<td>
                  				<img src='/stu/file/${row.GOODS_THUMBNAIL}' width="70px" height="70px">
                  			</td>
							<td>
				  				<a href="/stu/shop/goodsDetail.do?IDX=${row.GOODS_NO }">${item.itemname}</a> <br>
							...
							<td style="text-align:center">
								...
								<input type="text" name="itemcost" value="${item.itemcost}"style="width:60px; text-align:right; border:none;">원
							</td>
							<td style="text-align:center">
